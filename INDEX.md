# noBS CAD — repository index

| Path | Role |
|------|------|
| [README.md](README.md) | Public product overview |
| [docs/INDEX.md](docs/INDEX.md) | Documentation map |
| [docs/agentic/INDEX.md](docs/agentic/INDEX.md) | Agent/maintainer operating guides |
| [mcp-server/INDEX.md](mcp-server/INDEX.md) | MCP server surface & disclosure |
| [src/INDEX.md](src/INDEX.md) | Desktop UI entrypoints |
| [crates/INDEX.md](crates/INDEX.md) | Host-neutral Rust crates |
| [src-tauri/INDEX.md](src-tauri/INDEX.md) | Native shell (Tauri + OCCT) |

**Steerable MCP (branch):** soft focus-scoped tools, headless session attach,
3MF/STL export. See [docs/mcp-harness.md](docs/mcp-harness.md) and
[mcp-server/OKRs.md](mcp-server/OKRs.md).

**Follow-ups (not in this slice):** live UI co-link, MCP client installer.
