...
const catalog = JSON.parse(readFileSync(new URL('../interface/catalog.json', import.meta.url), 'utf8'));
function directCommands(workspaceId) {
  const workspace = catalog.workspaces.find((entry) => entry.id === workspaceId);
  assert.ok(workspace, `missing ribbon workspace: ${workspaceId}`);
...
try {
  ...
  assert.deepEqual(modelWide.buttons, directCommands('solid'), `wide Modeling ribbon should restore every direct command: ${JSON.stringify(modelWide)}`);
  ...
  assert.deepEqual(sketchWide.buttons, directCommands('sketch'), `wide Sketch ribbon should restore every direct command: ${JSON.stringify(sketchWide)}`);
  ...
  assert.deepEqual(drawingWide.buttons, directCommands('drawing'), `wide Drawing ribbon should restore every direct command: ${JSON.stringify(drawingWide)}`);
  ...
  const camPanels = ['workspace', 'setup', 'toolpaths'];
  ...
  assert.deepEqual(camWide.buttons, directCommands('cam'), `wide CAM ribbon should restore every direct command: ${JSON.stringify(camWide)}`);
  ...
  await page.setViewportSize({ width: 900, height: 900 });
  const simulatePanels = ['simulation', 'playback', 'display'];
  await waitForRibbon(simulatePanels);
  await assertCompactRibbon(simulatePanels, 'camSim');
  await page.getByRole('tab', { name: 'Output', exact: true }).click();
  await waitForRibbon(['output', 'advanced']);
  await assertCompactRibbon(['output', 'advanced'], 'postNc');
  assert.equal(await page.locator('[data-ribbon-button="postEvents"]').count(), 0);
