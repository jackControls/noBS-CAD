...
fn compare_solids_summary(scene: &SolidSceneDto) -> Value {
...
mod tests {
    ...
    fn assembly_component_occurrence_grounded_roundtrip() {
        ...
        assert!(
            definitions.iter().any(|definition| {
                definition["id"].as_u64() == Some(component_id) && definition["name"] == "Block"
            }),
    ...
    fn cad_script_after_attach_refresh_replays_on_fresh_server() {
        ...
        // #60 attach is session_read_only for direct mutates; detach forks headless so
        // subsequent modeling ops append to the seeded cad_script baseline.
        server
            .call_tool("cad_detach", json!({}))
            .expect("detach to allow headless mutate after seeded attach/refresh");
        ...
        let expected = server
            .call_tool("cad_compare_solids", json!({}))
            .expect("compare solids on seeded+modeled server");
    ...
    #[test]
    fn solid_import_and_move_copy_accepted_by_cad_submit_classifier() {
        for name in [
            "solid_import_step",
            "solid_edit_import_step",
            "solid_move_copy",
            ...
            assert!(
                nbcad_mcp_mutate::lookup_mutate(name).is_some(),
                "{name} must be in shared mutate map"
            );
            assert!(is_modeling_mutate(name), "{name}");
            assert!(!is_read_safe_while_attached(name), "{name}");
        }
        assert!(is_read_safe_while_attached("cad_script"));
        assert!(is_read_safe_while_attached("cad_compare_solids"));
        assert!(!is_modeling_mutate("cad_script"));
        assert!(!is_modeling_mutate("cad_compare_solids"));
        assert!(nbcad_mcp_mutate::lookup_mutate("cad_script").is_none());
        assert!(nbcad_mcp_mutate::lookup_mutate("cad_compare_solids").is_none());
    }
}
