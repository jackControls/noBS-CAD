...
import type {
  CamDocumentDto,
  CamDrillCycle,
  CamOperationHeightExpressionsDto,
  CamOperationDto,
  CamLinkingDto,
...
/** Legacy document-level preference for unbound dialogs. A bound setup uses
 * its own machine snapshot; neither setting changes neutral geometry. */
...
/** Machine-only edits preserve neutral motion and stock cache identity.
 * Controller compatibility is checked afresh by Rust on every NC export. */
export function setCamMachine(
  setupId: number, machine: CamMachineAssignmentDto | null, expectedDocument?: CamDocumentDto,
): Promise<void> {
  return enqueueCamUpdate((cam) => {
...
export function reorderCamItems(setupId: number | null, ids: number[]): Promise<void> {
  return enqueueCamUpdate((cam) => reorderedCamDocument(cam, setupId, ids));
}

export async function duplicateCamSetup(setupId: number): Promise<void> {
  let createdId = 0;
  await enqueueCamUpdate((cam) => {
...
export async function duplicateCamOperation(operationId: number): Promise<void> {
  let createdId = 0, setupId = 0;
  await enqueueCamUpdate((cam) => {
    const result = duplicatedCamOperation(cam, operationId);
...
export function deleteCamSetup(setupId: number): Promise<void> {
  return enqueueCamUpdate((cam) => {
    ...
    next.toolpath_generations = (next.toolpath_generations ?? []).filter(
      (generation) => !removedOperationIds.has(generation.operation_id),
    );
    next.height_expressions = (next.height_expressions ?? []).filter(
      (expressions) => !removedOperationIds.has(expressions.operation_id),
    );
    next.linking = (next.linking ?? []).filter((item) => !removedOperationIds.has(item.operation_id));
...
export function setActiveCamSetup(setupId: number): Promise<void> {
  if (useAppStore.getState().camDocument.active_setup_id === setupId) {
    useAppStore.setState({ selectedCamSetupId: setupId, selectedCamOperationId: null });
    return Promise.resolve();
  }
  return enqueueCamUpdate((cam) => {
    if (!cam.setups.some((setup) => setup.id === setupId)) return cam;
    const next = { ...cam, active_setup_id: setupId };
    return next;
  }, true);
}
...
export type CamOperationHeightExpressionsInput = Omit<
  CamOperationHeightExpressionsDto,
  'operation_id'
>;

export function addCamOperation(
  operation: CamOperationInput,
  heights?: CamOperationHeightExpressionsInput,
  linking?: CamLinkingDto | null,
  placement?: CamOperationPlacement,
): Promise<number> {
  let createdId = 0;
  let createdSetupId = 0;
  return enqueueCamUpdate((cam) => {
    const next = structuredClone(cam);
    const id = next.next_operation_id;
    createdId = id;
    const setup = insertCamOperation(next, { ...structuredClone(operation), id } as CamOperationDto, placement);
    createdSetupId = setup.id;
    next.active_setup_id = setup.id;
    if (linking) { next.linking ??= []; next.linking.push({ ...structuredClone(linking), operation_id: id }); }
    if (heights) {
      next.height_expressions ??= [];
      next.height_expressions.push({ ...structuredClone(heights), operation_id: id });
    }
 ...
 * after planning succeeds. The engine owns all geometry work and safety
 * comparison; this layer only serializes the document mutation with edits. */
export function regenerateCamOperation(operationId: number): Promise<void> {
  return enqueueCamEngineMutation((engine) => engine.camRegenerateOperation(operationId));
}

/** Re-plan every enabled operation in a setup as one coherent program and
 * capture fresh signatures only after the complete setup succeeds. */
export function regenerateCamSetup(setupId: number): Promise<void> {
  return enqueueCamEngineMutation((engine) => engine.camRegenerateSetup(setupId));
}
...
export function deleteCamOperation(operationId: number): Promise<void> {
  return enqueueCamUpdate((cam) => {
    const next = structuredClone(cam);
    next.linking = (next.linking ?? []).filter((item) => item.operation_id !== operationId);
    let changed = false;
    ...
    next.toolpath_generations = (next.toolpath_generations ?? []).filter(
      (generation) => generation.operation_id !== operationId,
    );
    next.height_expressions = (next.height_expressions ?? []).filter(
...
export function replaceCamOperation(
  ...
  return enqueueCamUpdate((cam) => {
    ...
    for (const setup of next.setups) {
      ...
      // Undefined preserves a caller's existing intent; null explicitly
      // restores Automatic fitting by removing the manual linking record.
      if (linking !== undefined) {
        next.linking = (next.linking ?? []).filter((item) => item.operation_id !== operationId);
        if (linking) next.linking.push({ ...structuredClone(linking), operation_id: operationId });
      }
      if (heights) {
        next.height_expressions ??= [];
        next.height_expressions = next.height_expressions.filter(
        ...
        next.height_expressions.sort((left, right) => left.operation_id - right.operation_id);
      }
      return next;
    }
    return cam;
...
export function camToolCompatible(
  ...
  switch (kind) {
    ...
    case 'contour2d':
      return (tool.kind === 'flat_end_mill' || tool.kind === 'ball_end_mill' || tool.kind === 'bull_nose_end_mill' || tool.kind === 'face_mill') && tool.center_cutting;
    case 'chamfer2d':
