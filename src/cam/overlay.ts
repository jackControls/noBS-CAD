...
import { cachedCutterMesh } from './cutter';
import { currentStageSimulation, type CamWorkpieceView } from './view';
export { simulationHasStockSurface } from './view';
import { CUT_LINE, RAPID_LINE, simulationPlaybackPathLayers, simulationPlaybackPose } from './simulationPath';
export { simulationPlaybackPathId, simulationPlaybackPathLayers, simulationPlaybackPose } from './simulationPath';
...
export function collectCamOverlay(state: CamOverlayState): CamOverlayLayers {
  ...
  if (state.camChainPick && state.camChainPick.entities.length > 0) {
    ...
    const hovered: number[] = [];
    const selected: number[] = [];
    const selectedKeys = new Set(state.camChainPick.chains?.flatMap(c => c.keys) ?? state.camChainPick.selectedKeys);
    const hoverKeys = new Set(state.camChainPick.hoverKeys ?? [state.camChainPick.hoverKey]);
    if (state.camChainPick.direction) layers.arrows.push({
      ...state.camChainPick.direction, color: LOOP_LINE_SELECTED, width: 3, xray: true,
    });
    for (const entity of state.camChainPick.entities) {
      const target =
        hoverKeys.has(entity.key)
          ? hovered
          : selectedKeys.has(entity.key)
            ? selected
            : rest;
      const points = entity.modelPoints;
      const count = entity.closed || entity.kind === 'circle' ? points.length : points.length - 1;
  ...
  if (stockVisible && !state.camDialogOpen) {
...
/** Selection is an end-state review. Keep the same final retract pose before
 * and after stock arrives; only explicit playback moves the cutter. */
export function selectedOperationToolPose(program: CamProgramDto | null, setup: CamSetupDto, operationId: number | null) {
  if (operationId === null || !program) return null;
  const operation = setup.operations.find((candidate) => candidate.id === operationId);
  if (!operation?.enabled) return null;
  const commands = selectedSectionCommands(program, setup, operationId);
  for (let index = commands.length - 1; index >= 0; index -= 1) {
    const command = commands[index];
    if (command.kind === 'rapid' || command.kind === 'linear' || command.kind === 'circular') {
      return { position: command.to, toolId: operation.tool_id };
    }
  }
  return null;
}

/** Static tool and feed-endpoint markers for a selected operation. */
function pushSelectedTool(
  ...
  const tip = selectedOperationToolPose(program, setup, operationId)?.position;

  const toModel = (point: Point3Dto) => setupPointToModel(point, setup.wcs);
  if (tip && state.renderPlaybackTool !== false) {
    pushToolAt(layers, setup, tool, tip);
  }
...
function pushToolAt(layers: CamOverlayLayers, setup: CamSetupDto, tool: CamToolDto, tip: Point3Dto) {
  const mesh = cachedCutterMesh(tool);
  if (!mesh) return;
  for (const [part, color] of [[mesh.cutter, TOOL_FLUTE_FILL], [mesh.shank, TOOL_SHANK_FILL]] as const) {
    if (!part.positions.length) continue;
    const positions: number[] = [], normals: number[] = [];
    for (let i = 0; i < part.positions.length; i += 3) {
      const p = setupPointToModel({ x: tip.x + part.positions[i], y: tip.y + part.positions[i + 1], z: tip.z + part.positions[i + 2] }, setup.wcs);
      positions.push(p.x, p.y, p.z);
      const x = part.normals[i], y = part.normals[i + 1], z = part.normals[i + 2];
      for (let axis = 0; axis < 3; ++axis) {
        normals.push(x * setup.wcs.x_axis[axis] + y * setup.wcs.y_axis[axis] + z * setup.wcs.z_axis[axis]);
      }
    }
    layers.triangles.push({ color, positions, normals, xray: false });
  }
}
...
function pushSimulationTimelinePath(
  ...
  layers.lines.push(...simulationPlaybackPathLayers(timeline, firstCommand));
}
