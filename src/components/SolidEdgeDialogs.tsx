...
import { Blend, LoaderCircle, Triangle, X } from 'lucide-react';
    ...
    void getEngine()
      .then(async (engine) => {
        ...
        const currentBody = current.selectedBody !== null
          ...
          : null;
        const initialBody = edit?.body_id ?? currentBody ?? 0;
        ...
        current.setModelingPickTarget(kind === 'fillet' ? 'fillet_edges' : 'chamfer_edges');
      })
      .catch((cause: unknown) => {
              ...
              : bodies.length === 0 ? <p className="text-xs text-mute">{t('solidEdge.noBodies')}</p>
                : <>
                  <ViewportSelectionField
                    testId={`solid-${kind}-edge-selection`}
                    clearTestId={`solid-${kind}-clear-edges`}
                    label={t('solidEdge.edges')}
                    status={edgeIds.length === 0
                      ? t('solidEdge.pickEdges')
                      : `${edgeIds.length} ${t(edgeIds.length === 1 ? 'solidEdge.edgeSelected' : 'solidEdge.edgesSelected')}${body ? ` · ${body.name}` : ''}`}
                    hint={t('solidEdge.selectionHint')}
                    active={modelingPickTarget === (kind === 'fillet' ? 'fillet_edges' : 'chamfer_edges')}
                    hasSelection={edgeIds.length > 0}
                    onActivate={() => setModelingPickTarget(kind === 'fillet' ? 'fillet_edges' : 'chamfer_edges')}
                    onClear={() => {
                      setSelectedEdges([]);
                      setSelectedBody(null);
                      setHoveredEdge(null);
                      setModelingPickTarget(kind === 'fillet' ? 'fillet_edges' : 'chamfer_edges');
                    }}
                  />
