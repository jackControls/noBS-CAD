...
import { cancelTimelineFeatureEdit, submitHole } from '../engine/controller';
import type {
  BodyDto,
  FaceDto,
  HoleBottomStyle,
  ...
  return (
    ...
    >
      <form data-testid="hole-dialog" onSubmit={submit} className="feature-dialog pointer-events-auto absolute right-5 top-[132px] flex max-h-[calc(100vh-190px)] w-80 flex-col overflow-hidden border border-edge bg-panel">
        ...
        <footer className="flex h-11 shrink-0 items-center justify-end gap-2 border-t border-edge bg-header px-3"><button type="button" onClick={cancel} disabled={busy} className="h-7 rounded border border-edge px-3 text-xs text-ink hover:bg-edge">{t('hole.cancel')}</button><button data-testid="hole-ok" type="submit" disabled={!canSubmit} className="h-7 rounded bg-accent px-3 text-xs font-semibold text-white disabled:opacity-40">{t('hole.ok')}</button></footer>
      </form>
    </div>
  );
}
