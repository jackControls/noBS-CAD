...
export function CamGcodeSimulationDialog({ initial, onClose, onRun }: Props) {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!source.trim()) {
      setError(t('cam.simulate.chooseOrPaste'));
      return;
    }
    onRun({
      kind: 'gcode',
  ...
  return (
    ...
    >
      <form
        ...
        onSubmit={submit}
        className="feature-dialog pointer-events-auto absolute right-5 top-[160px] flex max-h-[calc(100vh-218px)] w-[440px] flex-col overflow-hidden rounded border border-edge bg-panel shadow-2xl"
      >
        <header className="flex h-10 shrink-0 items-center gap-2 border-b border-edge px-3">
          <CamToolIcon id="camNcSimulate" size={18} />
          <span className="flex-1 text-xs font-semibold text-ink">{t('cam.simulate.title')}</span>
          <button
            type="button"
            onClick={onClose}
            className="rounded p-1 text-mute hover:bg-edge hover:text-ink"
            aria-label={t('cam.simulate.close')}
          >
            <X size={14} />
          </button>
        </header>
        <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-3">
          ...
          <div className="grid grid-cols-[1fr_150px] gap-2">
            <label className="block">
              <span className={CAM_DIALOG_LABEL}>{t('cam.simulate.program')}</span>
              <div className="flex gap-1.5">
                ...
                >
                  <FolderOpen size={13} /> {t('cam.simulate.open')}
                </button>
                <input
                  ref={pickerRef}
                  type="file"
            ...
            <label className="block">
              <span className={CAM_DIALOG_LABEL}>{t('cam.simulate.language')}</span>
              <select
                value={dialect}
                onChange={(event) => setDialect(event.target.value as CamGcodeDialectDto)}
                className={CAM_DIALOG_INPUT}
              >
                <option value="auto">{t('cam.simulate.dialectAuto')}</option>
                <option value="siemens828d">{t('cam.simulate.dialectSiemens828d')}</option>
                <option value="iso">{t('cam.simulate.dialectIso')}</option>
                <option value="fanuc">{t('cam.simulate.dialectFanuc')}</option>
                <option value="haas">{t('cam.simulate.dialectHaas')}</option>
              </select>
            </label>
          </div>
          <label className="block">
            <span className={CAM_DIALOG_LABEL}>{t('cam.simulate.controllerCode')}</span>
            <textarea
              value={source}
              onChange={(event) => setSource(event.target.value)}
              spellCheck={false}
              placeholder={t('cam.simulate.placeholder')}
        ...
        <footer className="flex shrink-0 items-center justify-end gap-2 border-t border-edge px-3 py-2.5">
          ...
          >
            <CamToolIcon id="camNcSimulate" size={15} /> {t('cam.simulate.buildSimulation')}
          </button>
        </footer>
      </form>
    </div>
