...
Use **`assembly_interference_check`** to ask whether retained solids **collide or
...
## Check vs class selection (false friends)

| Concept | Role |
|---------|------|
| **Hole/shaft class** | Design **intent** for a mating pair — always-gap, locate, or press. Chosen from function and process; standards are citation-only. |
| **Assembly interference check** | **Measured geometry** after joints and placements: do these instances overlap *now*? |

An intentional **press** (overlap at nominals) will often report interference volume
in CAD if both solids are modeled at nominal — that can be expected. Accidental
clash between unrelated parts is not. Read the report against **intent**, then
decide whether to move occurrences, change joints, or change solid geometry.
...
- Need a minimum air gap? Set `clearance_threshold_mm` and treat near-zero
  assembly clearance as a miss, or model an explicit gap feature.
- Need a press? Expect overlap at nominals; qualify with coupons / process
...
etc.) — open them for contractual designations. This page does **not** redistribute
selection tables.
