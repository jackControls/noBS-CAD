        ...
        match command {
            ...
            CamCommandDto::ToolChange {
                ...
                if profile.preload_next_tool {
                    if let Some(next_call) = siemens_next_preload_call(program, index, &tool_call) {
            ...
            CamCommandDto::CutterCompensationOn { left } => {
                // Pin the controller to the same normal approach/retract
                // behavior modeled by the simulator instead of inheriting a
                // prior KONT/KONTC/KONTT mode from machine state.
                comp_words = Some(if *left { "G41 NORM" } else { "G42 NORM" });
            }
            CamCommandDto::CutterCompensationOff => {
                comp_words = Some("G40");
            }
...
mod tests {
    ...
    fn in_control_contour_posts_g42_with_the_tool_register_and_cancels_with_g40() {
        ...
        assert!(posted.nc.contains("G1 G40 X2 Y0 Z-2 F600"));
    }
    ...
    fn siemens_post_applies_radius_compensation_without_a_register_word() {
        ...
        .unwrap();
        assert!(posted.nc.contains("G1 G42 NORM X5 Y5 Z-2 F600"));
        assert!(posted.nc.contains("G1 G40 X2 Y0 Z-2 F600"));
    }
