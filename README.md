...
Metadata hints — not a full pre-sliced project). Headless session directories
(`cad_list_sessions` / `cad_attach`) support read-only load from
`NBCAD_SESSION_DIR`; live UI co-link is not shipped.
